...
public class PanchayatWebsiteFacade {
    ...
    public String uploadImageGeneric(MultipartFile imageFile, CompressionQuality compressionQuality) {
        log.info("PanchayatWebsiteFacade.uploadImageGeneric called");
        ImageCompressionDTO uploadResult = processAndUploadImage(imageFile, compressionQuality);
        if (uploadResult != null) {
    ...
    /**
     * Process content item images: upload images to cloud storage and update content JSON
     * Images are matched to items with null/empty image fields in order
     * 
     * @param contentJson The content JSON string
     * @param contentItemImages List of multipart files to upload
     * @param compressionQuality Compression quality for images
    ...
    private String processContentItemImages(String contentJson, List<MultipartFile> contentItemImages, CompressionQuality compressionQuality) {
        ...
        try {
            ...
            for (int itemIndex : itemsNeedingImages) {
                ...
                if (imageFile.getSize() == 0) {
                    log.error("Image file at index {} is empty (0 bytes)", imageIndex);
                    imageIndex++;
                    continue;
                }
                ...
                try {
                    ...
                    // Don't increment imageIndex here as it's incremented at the end of the loop
                }
